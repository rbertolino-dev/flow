...
export function ServiceBulkImport({ onImport, isImporting = false }: ServiceBulkImportProps) {
  ...
  const downloadTemplate = () => {
    const templateData = [
      {
        ...
        'Descrição': 'Descrição do serviço exemplo',
        'Preço': 100.00,
      ...
      },
      {
        ...
        'Descrição': 'Outro serviço exemplo',
        'Preço': 250.50,
    ...
    XLSX.writeFile(wb, 'template-servicos.xlsx');
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Validar limite de 100 serviços
        if (jsonData.length > 100) {
          toast({
            title: 'Limite excedido',
            description: `A planilha contém ${jsonData.length} serviços. O limite é de 100 serviços por importação.`,
            variant: 'destructive',
          });
          setImportFile(null);
          return;
        ...
        if (errors.length > 0) {
          ...
          toast({
            ...
            description: `${preview.length} serviço(s) prontos para importar.`,
          });
        }
      } catch (error: any) {
        toast({
  ...
  const handleImport = async () => {
    if (importPreview.length === 0) {
      toast({
        title: 'Nenhum serviço para importar',
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Importe até 100 serviços de uma vez usando uma planilha Excel. Baixe o template, preencha com seus serviços e faça o upload.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4">
                ...
                />
                <label htmlFor="service-import-file">
                  <Button
                    type="button"
                    variant="outline"
                    asChild
              ...
              <p className="text-xs text-muted-foreground">
                Limite: 100 serviços por planilha. Formatos aceitos: .xlsx, .xls
              </p>
            </div>
            ...
            {importPreview.length > 0 && (
              <div className="space-y-2">
                <h4 className="font-semibold flex items-center gap-2">
                  ...
                  Serviços Prontos para Importar: {importPreview.length}
                </h4>
                <div className="max-h-40 overflow-y-auto border rounded p-2 bg-muted/30">
                  <div className="space-y-1 text-sm">
                    {importPreview.slice(0, 10).map((service, index) => (
                    ...
                    {importPreview.length > 10 && (
                      <p className="text-xs text-muted-foreground italic">
                        ... e mais {importPreview.length - 10} serviço(s)
                      </p>
                    )}
                  </div>
                </div>
            ...
            >
              {isImporting ? (
                ...
                <>
                  ...
                  Importar {importPreview.length} Serviço(s)
                </>
              )}
            </Button>
          </DialogFooter>
