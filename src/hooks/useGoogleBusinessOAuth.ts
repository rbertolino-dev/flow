...
export function useGoogleBusinessOAuth() {
  ...
  const initiateOAuth = async (accountName?: string) => {
    if (!activeOrgId) {
      toast({
        ...
        description: "Selecione uma organização antes de conectar o Google Meu Negócio.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const messageListener = (event: MessageEvent) => {
        // Aceitar mensagens do mesmo origin ou do Supabase
        ...
        if (event.data && event.data.type === "GOOGLE_BUSINESS_OAUTH_SUCCESS") {
          ...
          toast({
            ...
            description: "Sua conta do Google Meu Negócio foi conectada com sucesso.",
          });
