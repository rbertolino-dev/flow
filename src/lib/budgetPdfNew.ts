// MÓDULO NOVO DE GERAÇÃO DE PDF DE ORÇAMENTO - CRIADO DO ZERO
// Baseado EXATAMENTE no contractPdfGenerator.ts que funciona perfeitamente
// TODOS os títulos são strings literais diretas (sem variáveis, sem interpolação)
...
export async function generateBudgetPDF(options: BudgetPdfOptions): Promise<Blob> {
  ...
  const drawPageHeader = () => {
    ...
    // TÍTULO DIRETO - STRING LITERAL (sem variáveis, sem interpolação)
  ...
  if (budget.payment_methods && budget.payment_methods.length > 0) {
    ...
    doc.setFontSize(10);
    doc.setTextColor(0, 0, 0);
    doc.text('FORMA DE PAGAMENTO', leftInfoX, infoY);
    infoY += lineHeight;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
  ...
  if (budget.expires_at) {
    ...
    doc.setFontSize(10);
    doc.setTextColor(0, 0, 0);
    doc.text('INFORMACOES DE ENTREGA', rightInfoX, rightInfoY);
  ...
  if (budget.observations) {
    ...
    doc.setFontSize(9);
    doc.setTextColor(0, 0, 0);
    doc.text('OUTRAS INFORMACOES', margin, yPosition);
    doc.setFont('helvetica', 'normal');
    yPosition += lineHeight;
