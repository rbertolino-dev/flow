...
/** Gera e baixa template .xlsx com seletores (origem, booleanos, medida). */
export async function downloadAgilizeProdutosTemplate(): Promise<void> {
  ...
  opcoes.getColumn(1).values = ["origem_produto", ...AGILIZE_ORIGEM_OPTIONS];
  opcoes.getColumn(2).values = ["boolean", ...AGILIZE_BOOLEAN_OPTIONS];
  opcoes.getColumn(3).values = ["medida", ...AGILIZE_MEDIDA_OPTIONS];
  opcoes.getColumn(1).width = 70;
  opcoes.getColumn(2).width = 12;
  opcoes.getColumn(3).width = 16;

  // --- Legenda ---
  legenda.addRow(["campo", "tipo", "obrigatorio", "opcoes_ou_formato", "descricao"]);
  for (const f of AGILIZE_TEMPLATE_FIELDS) {
    ...
    legenda.addRow([
      ...
      m.options?.join(" | ") ||
        (m.kind === "number"
          ? "numero pt-BR (ex.: 1.234,56) — convertido na importação"
          : "texto livre"),
  ...
  const header = [...AGILIZE_TEMPLATE_FIELDS];
  ws.addRow(header);
  ...
  addList(origemIdx, `Opcoes!$A$2:$A$${1 + AGILIZE_ORIGEM_OPTIONS.length}`);
  addList(filhoIdx, `Opcoes!$B$2:$B$${1 + AGILIZE_BOOLEAN_OPTIONS.length}`);
  addList(desIdx, `Opcoes!$B$2:$B$${1 + AGILIZE_BOOLEAN_OPTIONS.length}`);
  addList(medidaIdx, `Opcoes!$C$2:$C$${1 + AGILIZE_MEDIDA_OPTIONS.length}`);
