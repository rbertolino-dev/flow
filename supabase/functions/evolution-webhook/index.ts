  ...
  try {
    ...
    if (event === 'connection.update') {
      ...
      if (isNowConnectedRaw !== null) {
        const wasConnected = configs.is_connected === true;
        const isNowConnected = isNowConnectedRaw === true;

        if (isNowConnected === configs.is_connected) {
          return new Response(
            JSON.stringify({
              success: true,
              instance,
          ...
          if (!qrCode && configs.api_url && configs.api_key && configs.instance_name) {
            try {
              ...
              if (qrResponse.ok) {
                ...
                // Se API retornar só "code" (pairing), não gerar imagem aqui (evita 500 no Deno); frontend gera na reconexão
              }
            } catch (qrError) {
    ...
    if (event === 'qrcode.updated') {
      ...
      if (configs && payload.qrcode) {
        ...
        if (!qrToSave.startsWith('data:image') && isBase64Image(qrToSave)) {
          qrToSave = `data:image/png;base64,${qrToSave}`;
        }
        // Se for pairing code (ex: "2@..."), salvar como está; frontend gera imagem ao exibir
        await supabase
          .from('evolution_config')
          .update({ 
            qr_code: qrToSave,
