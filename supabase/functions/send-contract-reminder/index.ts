...
async function sendWhatsAppReminder(reminder: any, contract: any, supabase: any): Promise<boolean> {
  try {
    ...
    const { data: evolutionConfig } = await supabase
      ...
      .select('instance_id, instance_name, api_key, api_url')
    ...
    // Enviar via Evolution API (usar instance_name ao invés de instance_id)
    const evolutionApiUrl = evolutionConfig.api_url.replace(/\/$/, '');
    const instanceName = evolutionConfig.instance_name || evolutionConfig.instance_id;
