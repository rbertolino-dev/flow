#!/bin/bash
# Aplica migration PDV no PostgreSQL Hetzner (budget_services)
set -euo pipefail
...
MIGRATION_FILE="$SCRIPT_DIR/../supabase/migrations/20260322120000_create_pos_tables_postgres.sql"
TEMP_FILE="/tmp/create_pos_tables_postgres.sql"
...
echo "🚀 Aplicando migration PDV no PostgreSQL Hetzner..."
...
echo "✅ Migration PDV aplicada"
sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no "$SSH_USER@$SSH_HOST" \
  "export PGPASSWORD='$POSTGRES_PASSWORD' && psql -h $POSTGRES_HOST -p $POSTGRES_PORT -U $POSTGRES_USER -d $POSTGRES_DB -c \"SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name LIKE 'pos_%' ORDER BY 1;\""
