#!/bin/bash
# Aplica migration sold_at/supplier_name no PostgreSQL Hetzner (budget_services)
set -euo pipefail
...
MIGRATION_FILE="$SCRIPT_DIR/../supabase/migrations/20260322140000_pos_sales_sold_at_supplier.sql"
TEMP_FILE="/tmp/pos_sales_sold_at_supplier.sql"
...
echo "🚀 Aplicando migration PDV sold_at/supplier no PostgreSQL Hetzner..."
...
if [ -z "${POSTGRES_PASSWORD:-}" ]; then
  echo "❌ Senha PostgreSQL não encontrada em /root/postgresql-budget-credentials.txt"
  exit 1
fi
...
echo "✅ Migration sold_at/supplier aplicada"
sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no "$SSH_USER@$SSH_HOST" \
  "export PGPASSWORD='$POSTGRES_PASSWORD' && psql -h $POSTGRES_HOST -p $POSTGRES_PORT -U $POSTGRES_USER -d $POSTGRES_DB -c \"SELECT column_name, data_type FROM information_schema.columns WHERE table_name='pos_sales' AND column_name IN ('sold_at','supplier_name') ORDER BY 1;\""
