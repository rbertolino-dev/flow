...
# Script para aplicar migration de melhorias de contratos diretamente via SQL
# Usa Supabase Management API para executar SQL diretamente
...
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Aplicar Migration Contratos (Direto)  ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""
...
# Aplicar via Supabase CLI usando link + db push
...
if echo "y" | timeout 120 supabase db push --include-all 2>&1 | tee /tmp/migration_contract_improvements_direto.log; then
    echo ""
    ...
    tail -30 /tmp/migration_contract_improvements_direto.log
    echo ""
