...
--
-- IMPORTANTE: 
-- 1. Substituir [SERVICE_ROLE_KEY] pela chave real
-- 2. Substituir [PROJECT_URL] pela URL do projeto
...
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    url := '[PROJECT_URL]/functions/v1/sync-daily-metrics',
...
SELECT cron.schedule(
  ...
  $$
  SELECT net.http_post(
    url := '[PROJECT_URL]/functions/v1/process-whatsapp-workflows',
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer [SERVICE_ROLE_KEY]'
    ),
...
SELECT cron.schedule(
  ...
  $$
  SELECT net.http_post(
    url := '[PROJECT_URL]/functions/v1/process-scheduled-messages',
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer [SERVICE_ROLE_KEY]'
    ),
...
SELECT cron.schedule(
  ...
  $$
  SELECT net.http_post(
    url := '[PROJECT_URL]/functions/v1/process-status-schedule',
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer [SERVICE_ROLE_KEY]'
    ),
...
SELECT cron.schedule(
  ...
  $$
  SELECT net.http_post(
    url := '[PROJECT_URL]/functions/v1/sync-google-calendar-events',
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer [SERVICE_ROLE_KEY]'
    ),
...
SELECT cron.schedule(
  ...
  $$
  SELECT net.http_post(
    url := '[PROJECT_URL]/functions/v1/process-google-business-posts',
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer [SERVICE_ROLE_KEY]'
    ),
